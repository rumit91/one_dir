...
def createUser(login,passwd):
    ...
    with open('userinfo.pickle', 'rb') as handle:
        reader = pickle.load(handle)
        if reader.has_key(login):   return False
        else:   tmp = True
    if tmp == True:
...
while True:
    ...
    if command == 'check':
        ...
        if matchpasswd(username, password):     print 'Successfully Logged in\n'
        ...
        if createUser(usernameCreate, passwordCreate):  print 'User Created\n'
        else:   print 'ERR. User exists\n'
    elif command == 'exit':
        break
